    ...
    return ( 
        <div className="flex flex-col gap-20 lg:gap-0 lg:grid lg:grid-cols-2 h-full">
            <div className="flex justify-center items-center px-8 flex-col gap-12">
                ...
                <Button1 name={lang?.homepage.button} textColor={"#13637E"} textSize={"1rem"} borderColor={""} backgroundColor={"#E6E4D9"} url={"./servicepage"}/>
            </div>
            <div className="flex flex-col justify-center items-center gap-20">
                <div className={`w-[70%] lg:w-[30rem] ${styles.btnOurTrust1}`}>
                    <p className={`${ibmPlexSans.className} text-[1.2rem] font-normal opacity-90`}>{lang?.homepage.ipotesi1}</p>
                    <h2 className={`${ebGaramond.className} [font-size:_clamp(2rem,2.5vw,2.5rem)] font-semibold`}>{lang?.homepage.comprando2} <br />{lang?.homepage.comprando3}</h2>
                </div>
                <div className="flex justify-center flex-col px-4 lg:w-[30rem]">
                    ...
                    <ul className={`${ibmPlexSans.className} ${styles.bulletLine} [font-size:_clamp(1.5rem,1.8vw,1.8rem)] font-normal leading-[1.5] pl-6`}>
                        <li className="list-disc"><b>{lang?.homepage.docum}</b> {lang?.homepage.buroc}</li>
                        <li className="list-disc"><b>{lang?.homepage.cont}</b> {lang?.homepage.invest} </li>
                        <li className="pe-0 sm:pe-[50%] lg:pe-20 list-disc"><b>{lang?.homepage.assist}</b><br/>{lang?.homepage.artigiani}</li>
                    </ul>
                </div>
            </div>
        </div>
